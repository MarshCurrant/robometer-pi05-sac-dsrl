...
class RemoteEnv(gym.Env):
    ...
    def __init__(
        self,
        server_url: str,
        obs_format: str = "droid",  # "droid", "widowx", or "robotwin"
        socket_timeout: float = 30.0,
        connect_timeout: float = float("inf"),
        retry_interval: float = 5.0,
        num_stages: int = 1,
        dsrl_image_keys: Optional[list[str]] = None,
    ):
        """
        Args:
        ...
        self.dsrl_image_keys = list(dsrl_image_keys) if dsrl_image_keys is not None else None
        self.stage_idx = 0
        ...
        elif obs_format == "robotwin":
            self.image_resolution = ROBOTWIN_IMAGE_RESOLUTION
        else:
        ...
        if obs_format == "droid":
            ...
            image_keys = self.dsrl_image_keys or ["observation.images.wrist_pair"]
            valid_image_keys = {
            ...
            invalid_image_keys = set(image_keys) - valid_image_keys
            if invalid_image_keys:
                raise ValueError(f"Unsupported RoboTwin DSRL image keys: {sorted(invalid_image_keys)}")
            self.dsrl_key_mapping = {
                "image": image_keys,
                "state": "state",
                "language": "language",
            }
            image_space = lambda key: gym.spaces.Box(
                low=0,
                high=255,
            ...
            self.observation_space = gym.spaces.Dict(
                {
                    "observation.images.cam_high": image_space("observation.images.cam_high"),
                    "observation.images.cam_left_wrist": image_space("observation.images.cam_left_wrist"),
                    "observation.images.cam_right_wrist": image_space("observation.images.cam_right_wrist"),
                    "observation.images.wrist_pair": image_space("observation.images.wrist_pair"),
                    ...
                    "prompt": gym.spaces.Text(min_length=1, max_length=512),
                }
            )
            self.action_dim = 14
        else:
    ...
    def step(self, action: np.ndarray) -> Tuple[Dict[str, Any], float, bool, bool, Dict[str, Any]]:
        ...
        try:
            ...
            if done or truncated:
                ...
                source = info.get("termination_source", "unspecified")
                print(
                    f"[RemoteEnv][termination={source}] Episode ended: "
                    f"{status} (reward={reward:.1f})"
                )
    ...
    def _get_dummy_observation(self) -> Dict[str, Any]:
        ...
        if self.obs_format == "droid":
            ...
            stage_feature = self._stage_feature()
            image = np.zeros((self.image_resolution, self.image_resolution, 3), dtype=np.uint8)
            return {
                ...
                "observation.images.wrist_pair": image.copy(),
                "observation.state": np.zeros(14, dtype=np.float32),
                "state": np.concatenate([np.zeros(14, dtype=np.float32), stage_feature]),
                "prompt": "",
            }
    ...
    def _format_observation(self, obs: Dict[str, Any]) -> Dict[str, Any]:
        ...
        elif self.obs_format == "robotwin":
            policy_state = np.asarray(obs["observation.state"], dtype=np.float32)
            return {
                "observation.images.cam_high": np.asarray(obs["observation.images.cam_high"], dtype=np.uint8),
                "observation.images.cam_left_wrist": np.asarray(
                    obs["observation.images.cam_left_wrist"], dtype=np.uint8
                ),
                "observation.images.cam_right_wrist": np.asarray(
                    obs["observation.images.cam_right_wrist"], dtype=np.uint8
                ),
                "observation.images.wrist_pair": np.asarray(
                    obs["observation.images.wrist_pair"], dtype=np.uint8
                ),
                "observation.state": policy_state,
                "state": np.concatenate([policy_state, stage_feature]),
                "prompt": obs["prompt"],
            }
        else:
            return obs
