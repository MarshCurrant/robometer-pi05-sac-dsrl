...
class DatasetPreprocessor:
    ...
    def _load_dataset_from_path(self, dataset_path: str, subset: str):
        ...
        if "/" in dataset_path and not os.path.exists(dataset_path):
            ...
            # Load from local disk.  Dataset.save_to_disk() outputs are common
            # for offline finetuning runs; load_dataset(path) does not load
            # those reliably, so prefer Dataset.load_from_disk when present.
            if os.path.isdir(dataset_path) and (
                os.path.exists(os.path.join(dataset_path, "state.json"))
                or os.path.exists(os.path.join(dataset_path, "dataset_info.json"))
            ):
            ...
            # Local generated datasets store relative video paths in `frames`.
            # Convert them to absolute paths for the threaded decord fast path.
            if isinstance(dataset, Dataset) and "frames" in dataset.column_names:
                local_root = os.path.dirname(os.path.abspath(dataset_path))

                def patch_local_path(example):
                    frame_path = example.get("frames")
                    if not isinstance(frame_path, str):
                        return {}
                    if os.path.isabs(frame_path):
                        resolved = frame_path
                    else:
                        resolved = os.path.join(local_root, frame_path)
                    return {"frames_video": resolved, "frames_path": resolved}
