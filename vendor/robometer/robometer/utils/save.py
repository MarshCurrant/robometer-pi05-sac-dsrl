...
def _save_trainer_checkpoint_files(
    ...
    rank_zero = is_rank_0()
    fsdp_collective_save = bool(getattr(trainer, "is_fsdp_enabled", False))
    if not rank_zero and not fsdp_collective_save:
        logger.debug("Skipping checkpoint save on non-zero rank")
        return

    if rank_zero:
        os.makedirs(ckpt_dir, exist_ok=True)
    if fsdp_collective_save and torch.distributed.is_initialized():
...
def update_cfg_with_pretrained_ckpt(
    ...
    # Replace the model config except run-environment choices; sync only the
    # progress-loss and multi-image fields. The base model path and use_unsloth
    # depend on the current machine/cache and must remain controlled by launch.
...
def load_model_from_hf(
    ...
    if use_unsloth is not None and hasattr(exp_config, "model"):
        exp_config.model.use_unsloth = bool(use_unsloth)
    base_model_override = os.environ.get("ROBOMETER_BASE_MODEL_ID")
    if base_model_override and hasattr(exp_config, "model"):
        logger.info(f"Overriding RoboMeter base_model_id with ROBOMETER_BASE_MODEL_ID={base_model_override}")
