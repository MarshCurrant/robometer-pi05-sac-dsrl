...
class Logger:
    ...
    def __init__(
        ...
        raw_allowlist = os.environ.get("ROBOMETER_WANDB_METRIC_ALLOWLIST", "")
        self._wandb_metric_allowlist = [
            item.strip() for item in raw_allowlist.replace("\n", ",").split(",") if item.strip()
        ]
    ...
    def log_scalars(self, scalars: Dict[str, float], step: Optional[int] = None):
        ...
        if self.enabled("wandb"):
            wandb_scalars = self._filter_wandb_scalars(scalars)
            if wandb_scalars:
                self._wandb_run.log(wandb_scalars, step=step)
        if self.enabled("tensorboard"):
            for k, v in scalars.items():
                if isinstance(v, (int, float)):
                    self._tb_writer.add_scalar(k, float(v), global_step=step)

    def _filter_wandb_scalars(self, scalars: Dict[str, float]) -> Dict[str, float]:
        if not self._wandb_metric_allowlist:
            return scalars
        filtered = {}
        for key, value in scalars.items():
            if any(fnmatch.fnmatch(key, pattern) for pattern in self._wandb_metric_allowlist):
                filtered[key] = value
        return filtered
