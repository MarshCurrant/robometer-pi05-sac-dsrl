...
def _load_base_model_with_unsloth(
    ...
    if FastVisionModel is None:
        raise ImportError(
            "RoboMeter was configured with model.use_unsloth=True, but the "
            "`unsloth` package is not installed. Install unsloth or set "
            "model.use_unsloth=False for standard Transformers loading."
        )
...
def _load_base_model_standard(
    ...
    is_qwen3_requested = "Qwen3" in cfg.base_model_id or "qwen3" in cfg.base_model_id.lower()
    is_qwen3 = is_qwen3_requested and HAS_QWEN3
    if is_qwen3_requested and not HAS_QWEN3:
        raise RuntimeError(
            "RoboMeter config requests a Qwen3-VL base model "
            f"({cfg.base_model_id!r}), but this transformers installation does not expose "
            "Qwen3VLModel/Qwen3VLForConditionalGeneration. Upgrade transformers or use a "
            "checkpoint/config with a Qwen2.5-VL base model. Refusing to instantiate Qwen3-VL "
            "with Qwen2.5-VL classes because that silently changes model semantics."
        )
    ...
    if is_molmo:
        ...
        if cfg.model_type == "default":
            # Qwen3-VL instruct checkpoints are saved as
            # Qwen3VLForConditionalGeneration with weights under the "model."
            # prefix. Loading them directly as Qwen3VLModel leaves the whole
            # base model newly initialized, so load the wrapper and unwrap.
            qwen_model_cls = Qwen3VLForConditionalGeneration
            qwen_causal_model = qwen_model_cls.from_pretrained(
                cfg.base_model_id,
                torch_dtype=torch_dtype,
                **extra_kwargs,
                quantization_config=bnb,
            )
            base_model = qwen_causal_model.model
            logger.info("Using Qwen3 ForConditionalGeneration base model")
        else:
            qwen_model_cls = Qwen3VLForConditionalGeneration
            ...
            logger.info("Using Qwen3 ForConditionalGeneration models")
    else:
...
def setup_model_and_processor(
    ...
    valid_attention_backends = {"auto", "flash_attention_2", "sdpa", "eager"}
    if attention_override not in valid_attention_backends:
        raise ValueError(
            "ROBOMETER_ATTN_IMPLEMENTATION must be one of "
            f"{sorted(valid_attention_backends)}, got {attention_override!r}"
        )
    if attention_override == "auto":
        attention_implementation = "flash_attention_2" if has_flash_attn else "sdpa"
    else:
        attention_implementation = attention_override
    if attention_implementation == "flash_attention_2" and not has_flash_attn:
        raise RuntimeError(
            "ROBOMETER_ATTN_IMPLEMENTATION=flash_attention_2 was requested, but "
            "flash_attn is not importable"
        )
    logger.info(
        f"RoboMeter attention implementation: {attention_implementation} "
        f"(override={attention_override})"
    )
    extra_kwargs = {"attn_implementation": attention_implementation}
