...
LOG_LEVEL = os.environ.get("ROBOMETER_EVAL_LOG_LEVEL", "DEBUG")
...
def _autocast_context(device: str | torch.device, dtype_name: str | None):
    dtype_name = (dtype_name or "").lower()
    if dtype_name in {"", "none", "false", "off", "no"}:
        return nullcontext()
    try:
...
def _resolve_autocast_dtype(cfg: Any) -> str:
    env_value = os.environ.get("ROBOMETER_AUTOCAST_DTYPE")
    if env_value is not None and env_value.strip():
        return env_value.strip()
    return getattr(cfg, "autocast_dtype", "bf16")
...
def _resolve_use_unsloth(cfg: Any) -> bool | None:
    env_value = os.environ.get("ROBOMETER_USE_UNSLOTH")
    if env_value is not None:
        return _parse_optional_bool(env_value)
    return _parse_optional_bool(getattr(cfg, "use_unsloth", None))
...
def _sync_if_cuda(device: str | torch.device) -> None:
    try:
        ...
        parsed = torch.device("cuda" if str(device).startswith("cuda") else "cpu")
    if parsed.type == "cuda" and torch.cuda.is_available():
        torch.cuda.synchronize(parsed)


def _add_timing(timing: Dict[str, float], name: str, start: float) -> None:
...
def process_batch_helper(
    ...
    _add_timing(timing, "sample_dataclass_seconds", started)
    ...
    _add_timing(timing, "frame_step_expand_seconds", started)

    started = time.perf_counter()
    batch_inputs = batch_collator(input_samples)
    _add_timing(timing, "batch_collator_seconds", started)
    ...
    _add_timing(timing, "to_device_seconds", started)
    ...
    with _autocast_context(device, autocast_dtype):
        if num_preferences > 0:
            ...
            _add_timing(timing, "preference_outputs_total_seconds", started)
        ...
        if num_progress > 0:
            ...
            _add_timing(timing, "progress_outputs_total_seconds", started)
            ...
            _add_timing(timing, "aggregate_outputs_seconds", started)

    timing["helper_total_seconds"] = float(time.perf_counter() - total_start)
    return {
...
class MultiGPUEvalServer:
    ...
    def __init__(
        ...
        if self.autocast_dtype is None:
            self.autocast_dtype = os.environ.get("ROBOMETER_AUTOCAST_DTYPE", "bf16")
        self.use_unsloth = use_unsloth
        if self.use_unsloth is None:
            self.use_unsloth = _parse_optional_bool(os.environ.get("ROBOMETER_USE_UNSLOTH"))
        self._active_jobs = 0
        self._job_counter = 0
        ...
        logger.info(
            f"Loading experiment config and base model from {self.model_path}; "
            f"use_unsloth_override={self.use_unsloth}"
        )
    ...
    def _initialize_gpus(self):
        ...
        for gpu_id in range(self.num_gpus):
            ...
            # A one-GPU server does not need a second in-memory model replica.
            # Reusing the CPU-loaded objects avoids briefly holding two full
            # FFT checkpoints during startup. Multi-GPU servers still need one
            # independent object graph per device.
            if self.num_gpus == 1:
                tokenizer = self.base_tokenizer
                processor = self.base_processor
                model = self.base_model
                batch_collator = self.base_batch_collator
            else:
    ...
    async def process_batch(self, batch_data: List[Dict[str, Any]]):
        ...
        gpu_wait_seconds = time.perf_counter() - wait_start
        ...
        try:
            ...
            if isinstance(result, dict):
                timing = dict(result.get("_server_timing") or {})
                timing["gpu_wait_seconds"] = float(gpu_wait_seconds)
                timing["process_batch_total_seconds"] = float(time.time() - start_time)
                timing["gpu_id"] = int(gpu_info["gpu_id"])
                timing["active_jobs_at_acquire"] = int(active_jobs)
                timing["queue_size_after_acquire"] = int(queue_size_after_acquire)
                result["_server_timing"] = timing
...
def compute_batch_outputs(
    ...
    if device is not None:
        _sync_if_cuda(device)
    model_output, _ = forward_model(model, batch_inputs, sample_type=sample_type, timing_raw=timing_raw)
    if device is not None:
        _sync_if_cuda(device)
    _add_timing(timing_raw, f"{sample_type}_forward_model_seconds", started)
    ...
    _add_timing(timing_raw, f"{sample_type}_postprocess_seconds", started)
...
def create_app(cfg: EvalServerConfig, multi_gpu_server: MultiGPUEvalServer | None = None):
    ...
    multi_gpu_server = multi_gpu_server or MultiGPUEvalServer(
        ...
        use_unsloth=_resolve_use_unsloth(cfg),
    )
    ...
    async def evaluate_batch(batch: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.error(f"/evaluate_batch failed: {exc}\n{traceback.format_exc()}")
            raise HTTPException(status_code=500, detail={"error": type(exc).__name__, "message": str(exc)}) from exc
    ...
    async def evaluate_batch_npy(request: Request) -> Dict[str, Any]:
        ...
        try:
            ...
            sample_types = [sample.get("sample_type") for sample in batch_data if isinstance(sample, dict)]
            frame_shapes = []
            for sample in batch_data:
                if isinstance(sample, dict) and isinstance(sample.get("trajectory"), dict):
                    frames = sample["trajectory"].get("frames")
                    frame_shapes.append(tuple(frames.shape) if hasattr(frames, "shape") else type(frames).__name__)
            ...
            logger.debug(
                ...
                f"sample_types={sample_types} frame_shapes={frame_shapes}"
            )
            return await multi_gpu_server.process_batch(batch_payload)
        except Exception as exc:
            logger.error(f"/evaluate_batch_npy failed: {exc}\n{traceback.format_exc()}")
            raise HTTPException(status_code=500, detail={"error": type(exc).__name__, "message": str(exc)}) from exc
    ...
    def get_model_info() -> Dict[str, Any]:
        ...
        try:
            ...
            auto_attention_backend = "flash_attention_2"
        except Exception:
            flash_attn_importable = False
            auto_attention_backend = "sdpa"
        attention_override = os.environ.get(
            "ROBOMETER_ATTN_IMPLEMENTATION", "auto"
        ).strip().lower()
        attention_backend_expected = (
            auto_attention_backend
            if attention_override == "auto"
            else attention_override
        )
        ...
        return {
            ...
            "attention_backend_expected": attention_backend_expected,
            "attention_backend_override": attention_override,
...
def main(cfg: DictConfig):
    ...
    autocast_dtype = _resolve_autocast_dtype(eval_cfg)
    if hasattr(eval_cfg, "autocast_dtype"):
        eval_cfg.autocast_dtype = autocast_dtype
    use_unsloth = _resolve_use_unsloth(eval_cfg)
