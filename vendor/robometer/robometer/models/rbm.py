...
def _sync_timing_device(device: torch.device | str | None) -> None:
    if device is None:
        return
...
def _add_sync_timing(timing_raw: dict[str, float], name: str, start: float) -> None:
    timing_raw[name] = timing_raw.get(name, 0.0) + float(time.perf_counter() - start)
...
class RBM(PredictionHeadsMixin, PreTrainedModel):
    ...
    config_class = Qwen3VLModel.config_class if Qwen3VLModel is not None else Qwen2_5_VLModel.config_class
        ...
        **kwargs,
        ...
        if self.use_per_frame_progress_token:
            progress_logits, success_logits, pref_or_sim_logits = self._process_token_extraction(
                hidden_state, input_ids, sample_type, timing_raw
            )
    ...
    def _forward_qwen(
        ...
        **kwargs,
        ...
        _sync_timing_device(input_ids.device)
        qwen_sync_start = time.perf_counter()
        with _timer("time/rbm_forward", timing_raw=timing_raw):
        ...
        _sync_timing_device(input_ids.device)
        _add_sync_timing(timing_raw, "time/qwen_backbone_sync", qwen_sync_start)
    ...
    def _process_token_extraction(
        ...
        _sync_timing_device(hidden_state.device)
        token_sync_start = time.perf_counter()
        with _timer("time/token_extraction", timing_raw=timing_raw):
            all_prog_token_hidden = self._extract_hidden_state_from_token(hidden_state, input_ids, "<|prog_token|>")
        _sync_timing_device(hidden_state.device)
        _add_sync_timing(timing_raw, "time/token_extraction_sync", token_sync_start)

        if sample_type == "progress":
            # For progress samples, all tokens belong to trajectory A
            hidden_states_A = all_prog_token_hidden
            _sync_timing_device(hidden_state.device)
            heads_sync_start = time.perf_counter()
            with _timer("time/progress_success_heads", timing_raw=timing_raw):
                progress_logits["A"], success_logits["A"] = self._apply_heads_to_hidden_states(hidden_states_A)
            _sync_timing_device(hidden_state.device)
            _add_sync_timing(timing_raw, "time/progress_success_heads_sync", heads_sync_start)
        elif sample_type == "preference":
            # For preference, assume equal number of tokens for A and B
            with _timer("time/token_split", timing_raw=timing_raw):
                hidden_states_A = []
                hidden_states_B = []
            ...
            with _timer("time/preference_head", timing_raw=timing_raw):
                token_hidden = self._extract_hidden_state_from_token(hidden_state, input_ids, "<|pref_token|>")
