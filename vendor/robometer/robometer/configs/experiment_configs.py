...
class TrainingConfig:
    ...
    detect_anomaly: bool = field(
        default=False,
        metadata={"help": "Enable torch autograd anomaly detection. Very slow; use only for debugging."},
    )
    log_optimizer_stats: bool = field(
        default=False,
        metadata={"help": "Log expensive per-parameter optimizer/gradient statistics."},
    )
    log_memory_steps: int = field(
        default=0,
        metadata={"help": "Log detailed memory usage every N trainer steps. 0 disables per-step memory logging."},
    )
