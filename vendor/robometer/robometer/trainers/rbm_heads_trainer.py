...
class RBMHeadsTrainer(Trainer):
    ...
    def training_step(self, model, inputs, num_items_in_batch=None):
        ...
        memory_log_steps = int(getattr(self.config.training, "log_memory_steps", 0) or 0)
        if memory_log_steps > 0 and self.state.global_step % memory_log_steps == 0:
    ...
    def _get_optimizer_stats(self):
        ...
        if not bool(getattr(self.config.training, "log_optimizer_stats", False)):
            return optim_stats
    ...
    def _compute_success_loss_helper(
        ...
        # Compute BCE loss and apply class-balancing weights. Keep the weighted
        # reduction intact; the previous implementation divided by the weights
        # after applying them, which cancelled class balancing.
        ...
        success_loss = weighted_loss.sum() / (sample_weights.sum() + 1e-8)
        loss = raw_loss * combined_mask
    ...
    def _compute_progress_loss_helper(
        ...
        # Preserve the upstream RoboMeter training behavior: average each
        # sequence's frame loss first, then normalize by the valid-frame count.
        # This intentionally keeps progress loss smaller than an unnormalized
        # per-frame sum when it is combined with preference/success losses.
    ...
    def _compute_preference_loss(self, model, inputs, return_outputs=False, training=True):
        ...
        logger.trace(f"DATA GEN STRAT FOR TRAJ A: {data_gen_strat}")
        logger.trace(f"DATA SOURCE FOR TRAJ A: {inputs['trajectory_A_data_source']}")
