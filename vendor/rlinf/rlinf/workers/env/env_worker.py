...
class EnvWorker(Worker):
    ...
    def _prepare_trajectory_builders(
        ...
        compact_dsrl = bool(
            OmegaConf.select(
                self.cfg,
                "algorithm.replay_buffer.compact_dsrl",
                default=False,
            )
        )
        replay_observation_mode = "dsrl_compact" if compact_dsrl else "full"
        ...
        return [
            EmbodiedTrajectoryBuilder(
                max_episode_length=max_episode_length,
                replay_observation_mode=replay_observation_mode,
                replay_image_size=64,
            )
            for _ in range(self.stage_num)
        ]

    def _consume_robometer_async_metadata(
        self, stage_id: int, env_output: EnvOutput
    ) -> None:
        if env_output.env_infos is None:
            return
        metadata = env_output.env_infos.get("robometer_async_relabel")
        if not metadata:
            return
        builder = self.trajectory_builders[stage_id]
        tokens = metadata.get("submitted_tokens")
        if tokens:
            builder.register_async_reward_tokens(list(tokens))
        updates = metadata.get("updates") or []
        if updates:
            builder.apply_async_reward_updates(list(updates))

    def _flush_robometer_async_relabels(self, stage_id: int) -> None:
        flush = get_env_attr(self.env_list[stage_id], "flush_robometer_async")
        ...
        if updates:
            builder.apply_async_reward_updates(list(updates))
        builder.assert_async_rewards_complete()
        ...
        for epoch in range(self.rollout_epoch):
            ...
            for chunk_step_idx in range(self.n_train_chunk_steps):
                for stage_id in range(self.stage_num):
                    ...
                    self._consume_robometer_async_metadata(stage_id, env_output)
            ...
            for stage_id in range(self.stage_num):
                ...
                self._flush_robometer_async_relabels(stage_id)
