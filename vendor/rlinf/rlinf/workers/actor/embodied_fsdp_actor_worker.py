...
class EmbodiedFSDPActor(FSDPModelManager, Worker):
    ...
    def get_rollout_state_dict(self, syncable_only: bool = False) -> dict:
        if not syncable_only:
            return self.get_model_state_dict(cpu_offload=False, full_state_dict=False)

        # With FSDP NO_SHARD/use_orig_params, the wrapped module still owns the
        # full original tensors. Reading those references directly avoids the
        # FSDP state-dict hooks cloning every frozen parameter before the patch
        # syncer discards it.
        module = getattr(self.model, "module", self.model)
        named_tensors = dict(module.named_parameters(remove_duplicate=False))
        named_tensors.update(dict(module.named_buffers(remove_duplicate=False)))
        missing = [
            name for name in self.param_names_need_sync if name not in named_tensors
        ]
        if missing:
            raise RuntimeError(
                "Cannot build selective rollout state dict from wrapped model; "
                f"missing keys: {missing[:8]}"
            )
        return {name: named_tensors[name] for name in self.param_names_need_sync}

    def _supports_selective_rollout_state_dict(self) -> bool:
        return (
            self.cfg.weight_syncer.get("type") == "patch"
            and self.cfg.actor.fsdp_config.get("sharding_strategy") == "no_shard"
    ...
    async def sync_model_to_rollout(self) -> None:
        ...
        use_selective_state_dict = (
            self.weight_syncer.sender_initialized()
            and self._supports_selective_rollout_state_dict()
        )
        state_dict = self.get_rollout_state_dict(
            syncable_only=use_selective_state_dict
        )
