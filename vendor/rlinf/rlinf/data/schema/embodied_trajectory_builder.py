...
class EmbodiedTrajectoryBuilder:
    ...
    replay_observation_mode: str = "full"
    ...
    _async_reward_token_queue: list[list[str | None]] = field(
        default_factory=list, init=False, repr=False
    )
    _async_reward_locations: dict[str, tuple[int, int]] = field(
        default_factory=dict, init=False, repr=False
    )
    _async_reward_updates: dict[str, tuple[float, str]] = field(
        default_factory=dict, init=False, repr=False
    )
    _async_reward_unresolved: set[str] = field(
        default_factory=set, init=False, repr=False
    )

    def __post_init__(self) -> None:
        if self.replay_observation_mode not in {"full", "dsrl_compact"}:
            raise ValueError(
                "replay_observation_mode must be full or dsrl_compact, got "
                f"{self.replay_observation_mode!r}"
            )
        if self.replay_image_size <= 0:
            raise ValueError("replay_image_size must be positive")
    ...
    def append_step_result(self, result: ChunkStepResult):
        ...
        if result.rewards is not None:
            ...
            self._bind_next_async_reward_tokens(len(self.rewards) - 1)
    ...
    def append_transitions(self, curr_obs=None, next_obs=None):
        ...
        # These dictionaries are also the live observations retained by the env
        # worker for auto-reset bootstrapping. Removing prompt metadata in place
        # makes the next policy rollout receive ``prompt=None``.
        if self.replay_observation_mode == "dsrl_compact":
            stored_curr_obs = self._compact_dsrl_observation(curr_obs)
            stored_next_obs = self._compact_dsrl_observation(next_obs)
        else:
            stored_curr_obs = {
                key: value
    ...
    def _compact_dsrl_observation(self, obs: dict[str, Any]) -> dict[str, torch.Tensor]:
        main_images = obs.get("main_images")
        states = obs.get("states")
        if not isinstance(main_images, torch.Tensor) or not isinstance(
            states, torch.Tensor
        ):
            raise ValueError("DSRL replay requires tensor main_images and states")
        if main_images.ndim != 4:
            raise ValueError(
                "DSRL replay images must be [B,H,W,C] or [B,C,H,W], got "
                f"{tuple(main_images.shape)}"
            )
        if main_images.shape[-1] == 3:
            images = main_images.permute(0, 3, 1, 2)
        elif main_images.shape[1] == 3:
            images = main_images
        else:
            raise ValueError(
                f"Cannot identify RGB channel in replay image {tuple(main_images.shape)}"
            )
        images = images.cpu()
        ...
        images = F.interpolate(
            ...
            size=(self.replay_image_size, self.replay_image_size),
            mode="bilinear",
            align_corners=False,
        )
        return {
            "main_images": images.cpu().contiguous(),
            "states": states.cpu().contiguous(),
        }

    def register_async_reward_tokens(self, tokens: list[str | None]) -> None:
        if not tokens:
            raise ValueError("Async reward token batch cannot be empty")
        self._async_reward_token_queue.append(list(tokens))

    def _bind_next_async_reward_tokens(self, reward_index: int) -> None:
        if not self._async_reward_token_queue:
            return
        tokens = self._async_reward_token_queue.pop(0)
        reward_batch = self.rewards[reward_index]
        if len(tokens) != reward_batch.shape[0]:
            raise ValueError(
                "Async reward token batch does not match reward batch: "
                f"tokens={len(tokens)}, rewards={reward_batch.shape[0]}"
            )
        for env_index, token in enumerate(tokens):
            if token is None:
                continue
            if token in self._async_reward_locations:
                raise ValueError(f"Duplicate async reward token: {token}")
            self._async_reward_locations[token] = (reward_index, env_index)
            self._async_reward_unresolved.add(token)
            if token in self._async_reward_updates:
                reward, operation = self._async_reward_updates.pop(token)
                self._write_async_reward(token, reward, operation)

    def _write_async_reward(
        self, token: str, reward: float, operation: str = "replace"
    ) -> None:
        reward_index, env_index = self._async_reward_locations[token]
        target = self.rewards[reward_index][env_index]
        if operation == "replace":
            target.zero_()
            target[-1] = float(reward)
        elif operation == "add":
            target[-1] += float(reward)
        else:
            raise ValueError(
                f"Unsupported async reward operation for {token}: {operation!r}"
            )
        self._async_reward_unresolved.discard(token)

    def apply_async_reward_updates(self, updates: list[dict[str, Any]]) -> None:
        for update in updates:
            token = str(update["token"])
            reward = float(update["reward"])
            operation = str(update.get("operation", "replace"))
            if not np.isfinite(reward):
                raise ValueError(f"Non-finite async reward for {token}: {reward}")
            if token in self._async_reward_locations:
                self._write_async_reward(token, reward, operation)
            else:
                self._async_reward_updates[token] = (reward, operation)

    def assert_async_rewards_complete(self) -> None:
        if self._async_reward_token_queue:
            raise RuntimeError(
                f"{len(self._async_reward_token_queue)} async reward token batches are unbound"
            )
        if self._async_reward_updates:
            raise RuntimeError(
                f"{len(self._async_reward_updates)} async reward updates have no transition"
            )
        if self._async_reward_unresolved:
            raise RuntimeError(
                f"{len(self._async_reward_unresolved)} transitions are not relabeled"
            )
    ...
    def clear(self):
        ...
        self.curr_obs.clear()
        self.next_obs.clear()
        self._async_reward_token_queue.clear()
        self._async_reward_locations.clear()
        self._async_reward_updates.clear()
        self._async_reward_unresolved.clear()
