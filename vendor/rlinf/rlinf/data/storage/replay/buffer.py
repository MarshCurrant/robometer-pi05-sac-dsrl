...
class TrajectoryReplayBuffer:
    ...
    def _save_metadata(self, save_path: Optional[str] = None):
        ...
        with self._index_lock:
            metadata = {
                ...
                "seed": self.seed,
                # Sampling order is part of an exact training resume.  Saving
                # only the initial seed restarts the replay stream after every
                # checkpoint and silently changes the optimizer trajectory.
                "random_generator_state": self.random_generator.get_state().tolist(),
            }
            with open(self._get_metadata_path(save_path), "w") as f:
                json.dump(metadata, f)

    @staticmethod
    def _link_or_copy_file(source_path: str, target_path: str) -> None:
        """Checkpoint an immutable replay file without duplicating its payload.

        Auto-saved trajectory files are append-only.  A hard link therefore
        gives the checkpoint an independently recoverable directory entry while
        using no additional data blocks.  Cross-filesystem destinations fall
    ...
    def _checkpoint_trajectory_ids(self) -> list[int]:
        """Return the exact replay window represented by this checkpoint."""
        window_size = max(0, int(self.sample_window_size))
        with self._index_lock:
            if window_size > 0:
                return list(self._trajectory_id_list[-window_size:])
    ...
    def _load_trajectory(self, trajectory_id: int, model_weights_id: str) -> Trajectory:
        ...
        if self.trajectory_format == "pt":
            # Trajectory checkpoints are often hundreds of MiB because they hold
            # image observations.  Sampling only needs a small set of rows, so
            # mapping the tensor storages avoids copying every complete file for
            # every replay batch.  Advanced indexing below materializes exactly
            # the sampled rows and preserves the existing sampling semantics.
            try:
                trajectory_dict = torch.load(
                    trajectory_path,
                    map_location="cpu",
                    mmap=True,
                )
            except RuntimeError as exc:
                # Older checkpoints may use the legacy non-zip serialization,
                # which cannot be memory-mapped by torch.load.
                if "mmap" not in str(exc).lower():
                    raise
                self.logger.warning(
                    "Replay trajectory %s cannot be memory-mapped; falling back "
                    "to buffered loading: %s",
                    trajectory_path,
                    exc,
                )
    ...
    def sample_chunks(self, num_chunks: int) -> dict[str, torch.Tensor]:
        ...
        # 2) Cache misses: gather one trajectory at a time. Loading every
        # missed trajectory and concatenating them first can multiply host RAM
        # by the replay window size even though only ``num_chunks`` rows are
        # returned. Per-trajectory gathering preserves the sampled indices and
        # output order while bounding peak memory to one trajectory plus batch.
        ...
        if torch.any(miss_mask):
            ...
            for tid in miss_traj_ids:
                ...
                tid_mask = miss_mask & traj_ids_tensor.eq(tid)
                self._fill_batch_from_buffer_indices(
                    batch,
                    flat_trajectory,
                    local_sample_indices[tid_mask],
                    batch_indices_tensor[tid_mask],
                )
                del flat_trajectory, trajectory
    ...
    def save_checkpoint(self, save_path: str):
        ...
        if not self.auto_save:
            ...
            # Do not depend on sample_chunks() having populated the transient
            # window cache.  A checkpoint taken before the first training sample
            # must still contain every replay trajectory in its configured
            # window.
            for trajectory_id in self._checkpoint_trajectory_ids():
                ...
                save_futures.append(
                    self._checkpoint_executor.submit(
                        self._link_or_copy_file, trajectory_path, target_path
                    )
                )
    ...
    def load_checkpoint(
        ...
        if "seed" in metadata:
            ...
            self._init_random_generator(self.seed)
        random_generator_state = metadata.get("random_generator_state")
        if random_generator_state is not None:
            self.random_generator.set_state(
                torch.tensor(random_generator_state, dtype=torch.uint8)
            )
