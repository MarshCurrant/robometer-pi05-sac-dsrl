...
class PatchBuilder(ABC):
    ...
    def _validate_state_dict_keys(
        ...
        """Accept a full state dict or the exact selective-sync subset.

        The receiver ordinals are defined by the full state dict exchanged during
        initialization. Subsequent delta patches only inspect
        ``param_names_need_sync``, so requiring frozen tensors to be materialized
        again wastes memory without changing the generated patch.
        """
        state_keys = set(state_dict)
        ordered_keys = set(self.ordered_keys)
        sync_keys = self.param_names_need_sync_set
        if state_keys not in (ordered_keys, sync_keys):
            missing_sync_keys = sorted(sync_keys - state_keys)
            unexpected_keys = sorted(state_keys - ordered_keys)
            raise ValueError(
                "State dict keys do not match snapshot keys or the exact "
                "selective-sync subset; "
                f"missing_sync_keys={missing_sync_keys[:8]}, "
