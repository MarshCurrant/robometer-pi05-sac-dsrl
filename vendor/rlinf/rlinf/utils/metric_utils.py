...
def compute_evaluate_metrics(eval_metrics_list):
    ...
    for key in all_eval_metrics:
        ...
        if key == "init_state_id":
            aggregated_eval_metrics["unique_init_states"] = int(
                torch.unique(stacked).numel()
            )
            continue
    ...
    total_trajectories = int(sum(trajectory_counts))
    aggregated_eval_metrics["num_episodes"] = total_trajectories
    aggregated_eval_metrics["num_trajectories"] = total_trajectories
