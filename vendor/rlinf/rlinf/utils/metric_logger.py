...
class MetricLogger:
    ...
    def __init__(self, cfg: DictConfig):
        ...
        metric_allowlist = logger_cfg.get("metric_allowlist", None)
        self.metric_allowlist = (
            frozenset(str(key) for key in metric_allowlist)
            if metric_allowlist is not None
            else None
        )
        max_logged_metrics = int(logger_cfg.get("max_logged_metrics", 0))
        if (
            self.metric_allowlist is not None
            and max_logged_metrics > 0
            and len(self.metric_allowlist) > max_logged_metrics
        ):
            raise ValueError(
                f"metric_allowlist has {len(self.metric_allowlist)} entries, "
                f"exceeding max_logged_metrics={max_logged_metrics}"
            )
    ...
    def log(
        ...
        if self.metric_allowlist is not None:
            data = {
                key: value
                for key, value in data.items()
                if key in self.metric_allowlist
            }
            if not data:
                return
